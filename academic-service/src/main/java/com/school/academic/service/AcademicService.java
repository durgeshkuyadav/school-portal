...
public class AcademicService {
    ...
    public ExamResponse createExam(
            CreateExamRequest req, Long teacherId) {
    ...
    public List<ExamResponse> getExamsByClass(
            Long classId, String academicYear) {
    ...
    public List<ExamResponse> getExamsBySubjectAndClass(
            Long subjectId, Long classId) {
        return examRepository
            .findBySubjectIdAndClassId(subjectId, classId)
            .stream()
            .map(this::mapExamToResponse)
            .collect(Collectors.toList());
    }
    ...
    public ResultResponse saveResult(
        ...
        Exam exam = examRepository.findById(examId)
            .orElseThrow(() ->
                new RuntimeException("Exam not found: " + examId));
        ...
        if (req.getStatus() != null)
            result.setStatus(
                Result.ResultStatus.valueOf(req.getStatus()));
        return mapResultToResponse(resultRepository.save(result));
    }

    @Transactional
    public void bulkSaveResults(
            Long examId,
            List<SaveResultRequest> requests,
            Long teacherId) {
        requests.forEach(req ->
            saveResult(examId, req, teacherId));
    }
    ...
    public void publishResults(Long examId, Long teacherId) {
        ...
        // ✅ recipientEmail add kiya — test email
        eventProducer.publishResultsPublished(
            examId,
            exam.getClassId(),
            exam.getName(),
            "kpmedicalshop0@gmail.com"
        );
    }
    ...
    public StudentReportCard generateReportCard(
            Long studentId, String academicYear) {
        List<Result> results = resultRepository
            .findByStudentIdAndAcademicYear(studentId, academicYear);
        Map<String, List<Result>> bySubject = results.stream()
            .collect(Collectors.groupingBy(
                res -> res.getExam().getSubjectName()));
        List<SubjectSummary> summaries = bySubject.entrySet()
            .stream().map(entry -> {
                List<Result> subjectResults = entry.getValue();
                int total = subjectResults.stream()
                    .mapToInt(res -> res.getExam().getTotalMarks())
                    .sum();
                int obtained = subjectResults.stream()
                    .mapToInt(Result::getMarksObtained).sum();
                double pct = total > 0
                    ? (obtained * 100.0 / total) : 0;
        ...
        double overallPct = summaries.isEmpty() ? 0
            : summaries.stream()
                .mapToDouble(s -> s.getTotalMarks() > 0
                    ? s.getMarksObtained() * 100.0 / s.getTotalMarks()
                    : 0)
                .average().orElse(0);
    ...
    private ExamResponse mapExamToResponse(Exam e) {
        return ExamResponse.builder()
            ...
            .resultsPublished(e.isResultsPublished())
            .build();
    }
