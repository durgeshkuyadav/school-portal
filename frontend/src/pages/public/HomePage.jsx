...
export default function HomePage() {
  ...
  const [calState, setCalState] = useState({ y:2026, m:2 });
        ...
        <main className="site-main">
          ...
          <section id="events" className="sec">
            ...
            <div className="events-grid">
              {EVENTS.map((ev,i)=>(
                <div key={i} className={`ev-card ${ev.type}`} style={{animationDelay:`${i*.07}s`}}>
                  <div className="ev-date"><div className="ev-day">{ev.day}</div><div className="ev-month">{ev.month}</div></div>
                  <div className={`ev-tag ${ev.type}`}>{ev.type.charAt(0).toUpperCase()+ev.type.slice(1)}</div>
              ...
              {GALLERY.map((g,i)=>(
                <div key={i} className={`gal-item${hovGal===i?' hover':''}`}
                  ...
                  <div className="gal-inner">
                    <span className="gal-emoji">{g.e}</span>
                    <span className="gal-label">{g.label}</span>
                  </div>
                  <div className="gal-overlay"><span className="gal-overlay-txt">📷 {g.label}</span></div>
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="notif-list">
              {NOTIFS.map((n,i)=>(
                <div key={i} className={`notif${n.unread?' unread':''}${n.urgent?' urgent':''}`}
                  ...
                  <div className={`notif-ico ${n.type}`}>{n.ico}</div>
            ...
            <div className="teachers-scroll">
              {TEACHERS.map((t,i)=>(
                <div key={i} className="teacher-card" style={{animationDelay:`${i*.07}s`}}>
                  <div className="t-ava">{t.e}</div>
                  <div className="t-name">{t.name}</div>
                  <div className="t-sub">{t.sub}</div>
                  <span className="t-qual">{t.qual}</span>
                </div>
              ))}
            </div>
          </section>
